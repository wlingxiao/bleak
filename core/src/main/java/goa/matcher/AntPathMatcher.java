 ...
 * <li>{@code com/{filename:\\w+}.jsp} will canMatch {@code com/test.jsp} and assign the value {@code test}
 * to the {@code filename} variable</li>
 * </ul>
 * <p>
 * <p><strong>Note:</strong> a pattern and a path must both be absolute or must
 * both be relative in order for the two to canMatch. Therefore it is recommended
...
public class AntPathMatcher implements PathMatcher {
     ...
     * into this matcher's {@link #canMatch} method. A value of {@code true}
    ...
    public boolean canMatch(String pattern, String path) {
    ...
    /**
     * Actually canMatch the given {@code path} against the given {@code pattern}.
     *
     * @param pattern   the pattern to canMatch against
     * @param path      the path String to test
     * @param fullMatch whether a full pattern canMatch is required (else a pattern canMatch
    ...
    protected boolean doMatch(String pattern, String path, boolean fullMatch,
        ...
        if (pathIdxStart > pathIdxEnd) {
            // Path is exhausted, only canMatch if rest of pattern is * or **'s
     ...
     *
     * @param pattern the pattern to canMatch against (never {@code null})
     ...
     * <p>Assumes that {@link #canMatch} returns {@code true} for '{@code pattern}' and '{@code path}', but
    ...
    public Map<String, String> extractUriTemplateVariables(String pattern, String path) {
        ...
        if (!result) {
            throw new IllegalStateException("Pattern \"" + pattern + "\" is not a canMatch for \"" + path + "\"");
        }
        return variables;
    }
     ...
     * the first pattern contains a file extension canMatch (e.g., {@code *.html}).
    ...
    public String combine(String pattern1, String pattern2) {
        ...
        if (!pattern1.equals(pattern2) && !pattern1ContainsUriVar && canMatch(pattern1, pattern2)) {
                ...
                if (uriTemplateVariables != null) {
                    ...
                    if (this.variableNames.size() != matcher.groupCount()) {
                        throw new IllegalArgumentException("The number of capturing groups in the pattern segment " +
                                this.pattern + " does not canMatch the number of URI template variables it defines, " +
