...
import goa.{Cookies, Headers, Response, Status}
...
class EmbeddedClient(app: Netty) {
  ...
  def get(path: String,
          ...
          cookies: Seq[goa.Cookie] = Nil): Response = {
  ...
  private def perform(method: HttpMethod,
    ...
    val channel = new EmbeddedChannel(new DispatchHandler(app))
