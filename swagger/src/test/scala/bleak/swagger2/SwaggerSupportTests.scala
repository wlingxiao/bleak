...
class SwaggerSupportTests extends FunSuite with Matchers with BeforeAndAfter {
  ...
  test("to swagger") {
    ...
    val route1 = HttpRoute("/route1", Seq(Method.Get), "route1", Map.empty).asInstanceOf[Route]
  ...
  test("test swagger model") {
    val app = Mockito.mock(classOf[bleak.Application])
    val methods = Seq(Method.Get)
    val routes = ArrayBuffer(
      HttpRoute("/object", methods, "object", Map.empty),
      HttpRoute("/collection", methods, "collection", Map.empty),
      HttpRoute("/baseType", methods, "baseType", Map.empty)
    )
    Mockito.when(app.routes).thenReturn(routes.asInstanceOf[ArrayBuffer[Route]])
